...
def strategy_tendencies(df, symbl):
    ...
    try:
        ...
        for i, row in df.iterrows():
            ...
            if my_balance <= 0:
                return "strategy_tendencies", "RIP", round(my_balance, 2), row.ctmString

            if row['L_BB_L'] or row['C_BB_L']:
        ...
        margin_dates = df.ctmString[0], df.ctmString[len(df) - 1]
...
def prepare_data(crypto_symbol):
    ...
    """
    ohlc = client.klines(crypto_symbol, "1d", **{"limit": 1500})
    ...
    # df_day['RSI_OVER_70'] = np.where(df_day.RSI > 70, True, False)
    ...
    df_day['C_BB_L'] = np.where(df_day.close <= df_day.BB_L, True, False)

    # 7, 10, 6, 6
    # sto_k, sto_d = talib.STOCHRSI(df_day.close, timeperiod=14, fastk_period=5, fastd_period=3, fastd_matype=0)
